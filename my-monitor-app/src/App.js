...
const PullRequestList = () => {
  ...
  useEffect(() => {
    const fetchPullRequests = async () => {
      ...
      try {
        const response = await fetch('http://ec2-16-170-209-123.eu-north-1.compute.amazonaws.com:8000/api/v1/monitor_router/pull-requests');
        if (!response.ok) {
  ...
  const handleImageDownload = async (screenshotPath) => {
    ...
    const baseUrl = 'http://ec2-16-170-209-123.eu-north-1.compute.amazonaws.com:8000/api/v1/monitor_router/'; 
